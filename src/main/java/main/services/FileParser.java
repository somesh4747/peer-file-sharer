                            ...
                            if (filesInFolder == null || filesInFolder.length == 0) {
                                ...
                                for (File file : filesInFolder) {
                                    if (file.isFile()) {
                                        ...
                                        clientOutput.write("\n---END_OF_FILE---\n".getBytes());
                                        // clientOutput.flush(); // [ from GPT ] to clean outputStream after current file sent
                                        System.out.println("File " + fileName + " sent to the address " + clientSocket.getInetAddress());
                                    }
                                }
                            }
                        } catch (IOException e) {

                            System.out.println("file sending issue at " + port + " client address : " + clientSocket.getInetAddress());
